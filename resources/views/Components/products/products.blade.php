                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <td class="py-2 leading-5 px-2 text-center border-b">
                            <img src="{{ $product->image }}" alt="{{ $product->name }}" class="h-28 w-32 object-cover rounded-md mx-auto">
                        </td>
                        <td class="py-2 leading-5 px-2 text-center border-b">{{ $product->name }}</td>
                        <td class="py-2 leading-5 px-2 text-center border-b">{{ $product->categories->name ?? 'Không xác định' }}</td>
                        <td class="py-2 leading-5 px-2 text-center border-b">@php
                        echo str_replace('"', '', (Str::limit($product->description, 100)));
                        @endphp </td>
                        <td class="py-2 leading-5 px-2 text-center border-b">{{ number_format($product->price, 0, ',', '.') }} VNĐ</td>
