...
<div class="w-full p-4 mx-auto mb-4 rounded-lg shadow-lg">
    <h1 class="text-2xl font-semibold text-center mb-6">Sửa Phí Vận Chuyển</h1>

    <form action="{{ route('handleEditshipfee', $shippingFee->id) }}" method="POST">
        @csrf
        @method('PUT') <!-- Laravel method spoofing để thực hiện PUT request -->
        ...
        <div class="mb-4">
            ...
            <input 
                ...
                name="shipping_fee" 
                id="shipping_fee" 
                value="{{ old('shipping_fee', $shippingFee->shipping_fee) }}" 
                required 
        ...
        <div class="mb-4">
            ...
            <select 
                ...
                id="shipfee_type" 
                class="mt-1 px-4 py-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                <option value="nội tỉnh" {{ old('shipfee_type', $shippingFee->shipfee_type) == 'nội tỉnh' ? 'selected' : '' }}>nội tỉnh</option>
                <option value="liên tỉnh" {{ old('shipfee_type', $shippingFee->shipfee_type) == 'liên tỉnh' ? 'selected' : '' }}>liên tỉnh</option>
                <option value="vùng xa" {{ old('shipfee_type', $shippingFee->shipfee_type) == 'vùng xa' ? 'selected' : '' }}>vùng xa</option>
            </select>
            @error('shipfee_type')
        ...
        <div class="mb-4 flex items-center">
            <input 
                ...
                id="is_free_shipping" 
                {{ old('is_free_shipping', $shippingFee->is_free_shipping) ? 'checked' : '' }} 
        ...
        <div class="mb-4">
            ...
            <input 
                ...
                id="discount_amount" 
                value="{{ old('discount_amount', $shippingFee->discount_amount) }}" 
